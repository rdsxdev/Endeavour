    ...
    <>
      ...
      >
        <div className="surface flex items-center gap-2 px-4 py-2.5">
          ...
          <span className="text-sm text-mute">{live ? "Live chain data" : "Registry snapshot"}</span>
        </div>
      </PageHeader>
      ...
      <div className="mx-auto max-w-7xl px-5 py-10 lg:px-8">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <Reveal><Metric label="Credits issued" value={loading ? "—" : formatNumber(stats.totalCredits)} sub={`${formatCompact(stats.tonnesOffset)} tCO₂e`} /></Reveal>
          <Reveal delay={60}><Metric label="Verified" value={loading ? "—" : `${stats.verifiedPct}%`} sub={`${formatNumber(stats.verifiedCredits)} credits`} /></Reveal>
          <Reveal delay={120}><Metric label="Retired" value={loading ? "—" : formatNumber(stats.retiredCredits)} sub={`${formatCompact(stats.tonnesRetired)} tCO₂e offset`} /></Reveal>
          <Reveal delay={180}><Metric label="Active projects" value={loading ? "—" : formatNumber(stats.activeProjects)} sub={`${stats.countries} countries`} /></Reveal>
        </div>

        {/* Pool overview */}
        <Reveal className="mt-8">
          <Panel className="p-6">
            <h3 className="font-serif text-lg text-paper">Token pool overview</h3>
            <p className="mt-1 text-sm text-mute">Supply utilisation across asset classes</p>
            <div className="mt-6 grid gap-6 md:grid-cols-3">
              {POOLS.map((pool) => {
                const util = Math.round(((pool.totalSupply - pool.available) / pool.totalSupply) * 100)
                const color = poolColor(pool.kind)
                return (
                  <div key={pool.id} className="border-t-2 pt-4" style={{ borderColor: color }}>
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="font-mono text-xs text-mute">{pool.symbol}</p>
                        <p className="mt-0.5 font-medium text-paper">{pool.name}</p>
                      </div>
                      <Sparkline data={pool.trend} color={color} />
                    </div>
                    <p className="mt-3 font-mono text-xl text-paper">
                      {formatCompact(pool.totalSupply)}
                      <span className="ml-1 text-xs text-mute">{pool.unit}</span>
                    </p>
                    <div className="mt-3 h-1 bg-ink-3">
                      <div className="h-full" style={{ width: `${util}%`, background: color }} />
                    </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-3">
          <Reveal className="lg:col-span-2">
            <Panel className="p-6">
              <h3 className="font-serif text-lg text-paper">Issuance by vintage</h3>
              <p className="mt-1 text-sm text-mute">Credit volume per vintage year (tCO₂e)</p>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-3">
          <Reveal className="lg:col-span-2">
            <Panel className="p-6">
              <h3 className="font-serif text-lg text-paper">Geographic exposure</h3>
              <p className="mt-1 text-sm text-mute">Top issuing jurisdictions</p>
                ...
                {byCountry.map(([country, vol], i) => {
                  ...
                  return (
                    <div key={country} className="flex items-center gap-4">
                      <span className="w-8 font-mono text-xs text-mute">{String(i + 1).padStart(2, "0")}</span>
                      <span className="w-32 shrink-0 truncate text-sm text-paper">{country}</span>
                      <div className="h-2 flex-1 bg-ink-3">
                        <div className="h-full bg-emerald transition-all duration-700" style={{ width: `${pct}%` }} />
                      </div>
                      <span className="w-16 shrink-0 text-right font-mono text-xs text-mute">{formatCompact(vol)}</span>
                    </div>
                  )
                })}
              </div>
          ...
          <Reveal delay={80}>
            <Panel className="p-6">
              <h3 className="font-serif text-lg text-paper">CarbonPool trend</h3>
              <p className="mt-1 text-sm text-mute">6-month issuance velocity</p>
              <div className="mt-4">
                <AreaChart
              ...
              <dl className="mt-6 space-y-3 border-t border-line pt-5 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <dd className="font-mono text-paper">#{formatNumber(health?.latest_block ?? 0)}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-mute">Net impact</dt>
                  <dd className="font-mono text-emerald-deep">{formatCompact(stats.tonnesRetired)} tCO₂e</dd>
                </div>
              </dl>
            </Panel>
          </Reveal>
        ...
        <Reveal className="mt-6">
          <Panel className="p-6">
            <div className="flex items-center justify-between">
              ...
              <Link to="/registry" className="text-sm font-medium text-emerald-deep hover:text-emerald">
                View registry →
              </Link>
            </div>
...
function VintageChart({ data, loading }: { data: [number, number][]; loading: boolean }) {
  ...
  return (
    <div className="mt-8 flex h-52 items-end gap-3">
      {data.map(([year, vol]) => {
        ...
        return (
          <div key={year} className="group flex flex-1 flex-col items-center gap-2">
  ...
  return (
    <tr className="border-b border-line/60 last:border-0">
      <td className="py-3.5 pr-4">
        <Link to={`/credit/${credit.id}`} className="font-mono text-mute hover:text-emerald-deep">
          #{credit.id.toString().padStart(4, "0")}
        </Link>
      </td>
      <td className="py-3.5 pr-4 text-paper">{credit.project}</td>
      <td className="py-3.5 pr-4 font-mono text-mute">{shortAddress(credit.owner)}</td>
      <td className="py-3.5 pr-4 font-mono text-paper">{formatNumber(volumeOf(credit))}</td>
      <td className={`py-3.5 text-xs font-medium uppercase tracking-wide ${colors[status] ?? ""}`}>
        {status}
      </td>
    </tr>
  )
